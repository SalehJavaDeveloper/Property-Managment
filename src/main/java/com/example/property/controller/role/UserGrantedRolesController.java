...
public class UserGrantedRolesController {
    ...
    ResponseEntity<SuccessDetails<AuthenticationResponse>> saveUserGrantedRoles(@RequestBody UserGrantedRequestDto userGrantedRequestDto){

        return ResponseEntity.ok(new SuccessDetails<>( userGrantedRoleService.saveUserGrantedRole(userGrantedRequestDto), HttpStatus.OK.value(),true));
    }
