...
public class PermissionsServiceImpl implements PermissionService {
    ...
    @Override
    public void savePermission(PermissionRequestDto permissionRequestDto) {
        if(permissionRequestDto == null){
            throw new MethodArgumentNotValidException(("Data Arguments is not valid!"));
        }
        Permission permission = permissionMapper.fromDTO(permissionRequestDto);
        permission.setPacket(packetRepository.getReferenceById(permissionRequestDto.getPacketId()));
